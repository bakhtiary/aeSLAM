...
VisualOdometryMonoFlight::VisualOdometryMonoFlight (parameters param) : param(param), VisualOdometry((VisualOdometry::parameters)param) {
}

VisualOdometryMonoFlight::~VisualOdometryMonoFlight () {
}

bool VisualOdometryMonoFlight::process (uint8_t *I,int32_t* dims,bool replace) {
...
vector<double> VisualOdometryMonoFlight::estimateMotion (vector<Matcher::p_match> p_matched) {

  // get number of matches
  int32_t N = p_matched.size();
  if (N<10){
	 cout << "not enough initially matched features" ;
  ...
  if (!normalizeFeaturePoints(p_matched_normalized,Tp,Tc)){
	  cout << " could not normalize feature Points" ;
	  return vector<double>();
  }
  ...
  if (X_plane.n<10){
	  cout << "not enough points in front, bailing out" ;
	  return vector<double>();
  }
...
bool VisualOdometryMonoFlight::normalizeFeaturePoints(vector<Matcher::p_match> &p_matched,Matrix &Tp,Matrix &Tc) {
...
void VisualOdometryMonoFlight::fundamentalMatrix (const vector<Matcher::p_match> &p_matched,const vector<int32_t> &active,Matrix &F) {
...
vector<int32_t> VisualOdometryMonoFlight::getInlier (vector<Matcher::p_match> &p_matched,Matrix &F) {
...
void VisualOdometryMonoFlight::EtoRt(Matrix &E,Matrix &K,vector<Matcher::p_match> &p_matched,Matrix &X,Matrix &R,Matrix &t) {
...
int32_t VisualOdometryMonoFlight::triangulateChieral (vector<Matcher::p_match> &p_matched,Matrix &K,Matrix &R,Matrix &t,Matrix &X) {
